...
const DailyBill = ({ date, billList }) => {
  ...
  const dayResult = useMemo(() => {
    // 支出 收入 结余
    const pay = billList.filter(item => item.type === "pay").reduce((prev, curr) => prev + curr.money, 0)
    const income = billList.filter(item => item.type === "income").reduce((prev, curr) => prev + curr.money, 0)
